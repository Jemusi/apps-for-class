...
private:
   ...
   * You may want a recursive helper function for this one.
   ...
   void makeFourQuad(PNG* output, PNG im, int imSize);
   ...
   */
   ...
   bool hasNoChild(Node* curr);

   void render(PNG* img, Node* subRoot, int k);

   void prune(Node* subRoot, double tol);

   bool shouldPrune(Node* root, double tol, HSLAPixel rootAvg);

   int modInt(int a, int b);

   PNG* createPNG(PNG* im, int k, int x, int y);

   double getEntropy(int k, pair<int, int> ul, pair<int,int> lr, stats* s);
